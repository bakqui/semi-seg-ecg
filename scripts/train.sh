...
function set_options()
{
    arguments=$(getopt --options f:o:h \
                       --longoptions master_port:,gpus:,config_path:,override_config_path:,output_dir:,exp_name:,resume:,start_epoch:,help \
                       --name $(basename $0) \
                       -- "$@")
...
echo "Run segmentation model training..."
cd src
if [ $NUM_GPUS -gt 1 ]; then
    torchrun $TORCHRUN_ARGS train.py $MAIN_ARGS
else
